...
| Файл                                                    | Содержание               |
| ------------------------------------------------------- | ------------------------ |
| [.planning/state.md](state.md)                          | Текущая неделя, next     |
...
**Текущий этап:** развёртывание **завершено** (этапы A–F). Обучение: **недели 1–5 ✓**.
**Текущая неделя:** [6 — Advanced CRUD + Projects](../docs/roadmap-12-weeks.md#недели-5-12-коротко-с-акцентом-на-cj) (Projects, пагинация, filters).

## Стек

| Слой     | Сейчас                                                | План                |
| -------- | ----------------------------------------------------- | ------------------- |
| Frontend | Nuxt 4.4, Vue 3, SCSS, `useApi` / `useApiFetch`       | + Nuxt UI (нед. 10) |
| Backend  | Nitro, `server/api/*`, `apiHandler`, Zod validation   | Projects (нед. 6)   |
| БД       | PostgreSQL + Prisma (Docker)                          | relations, migrate  |
| Auth     | Better Auth + `@onmax/nuxt-better-auth`, RBAC         | OAuth (позже)       |
| API      | Unified `{ success, data, error? }` для `/api/tasks*` | notebook — legacy   |
...
| `server/utils/`             | Логика вне handlers (health, tasks, response, apiHandler, validation, prisma) |
| `shared/`                   | **Не удалять** — isomorphic типы/utils (`#shared`), client + server           |
| `types/`                    | Augmentation Nuxt (`nuxt-public.d.ts`)                                        |
| `shared/types/`             | Контракты API (`HealthResponse`, `ApiResponse`, `Task`, …)                    |
| `shared/validations/`       | Zod-схемы (`task.ts`) + `z.infer` типы                                        |
| `server/types/`             | Server-only типы — с нед. 2+                                                  |
...
- Клиент: только `useApi` / `useApiFetch` (unified API разворачивается в `data`; legacy routes — как есть).
- `/api/tasks*` — `{ success, data, error? }` через `apiHandler`.
- Не коммитить `.env`.

## Ограничения для агента

- Минимальный diff, без лишнего рефакторинга
- Prisma/Docker/PostgreSQL — только по [неделе 2](../docs/roadmap-12-weeks.md#неделя-2--prisma--postgresql--task-crud) и далее по roadmap
