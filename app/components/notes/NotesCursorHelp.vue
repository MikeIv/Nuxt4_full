...
import { CURSOR_HELP_CARDS } from '#shared/constants/cursorHelp'

const DETAIL_PANEL_ID = 'cursor-help-detail'
...
const activeCard = computed(() =>
  CURSOR_HELP_CARDS.find((card) => card.id === activeCardId.value),
)
</script>
...
<template>
  <article :class="$style.article">
    <header :class="$style.header">
      <h2 :class="$style.title">Cursor справка</h2>
      <p :class="$style.hint">
        Команды, шаблоны и workflow агента — раскройте карточку для подробностей
      </p>
    </header>

    <ul :class="$style.cards">
      <li v-for="card in CURSOR_HELP_CARDS" :key="card.id">
        <button
          type="button"
        ...
        <table v-if="section.table" :class="$style.table">
          <thead>
            <tr>
              ...
              <th :class="$style.th">Что писать</th>
            </tr>
          </thead>
          <tbody>
